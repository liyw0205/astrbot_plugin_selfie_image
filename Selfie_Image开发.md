...
2026-07-05：继续增强 provider HTML 文本响应解析，`extract_image_urls_from_text()` 支持 `<meta ... content="...">` 里的图片候选，可从 `og:image`、`twitter:image` 等 meta 标签中提取相对结果图或绝对图片 URL；普通非图片 `content` 仍会被过滤。相对路径继续按 provider base URL 解析下载，下载后走 Content-Type、大小和图片签名校验。补充 HTML meta 图片 content 单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只扩大 provider 响应解析兼容面。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider HTML 文本响应解析，`collect_images_from_unknown()` 会尝试解析 `<script ...>...</script>` 中的 JSON 对象或数组，可从 `application/json`、`ld+json` 等脚本块里提取相对图片路径或绝对图片 URL；普通 JavaScript 不会执行，只在内容可按 JSON 解析时递归识别。补充 HTML JSON script 图片 content 单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只扩大 provider 响应解析兼容面。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider HTML script 响应解析，`collect_images_from_unknown()` 支持 `window.__DATA__ = {...};` 这类脚本内 JSON 赋值形态，会提取等号后的 JSON 对象或数组并递归识别图片字段；仍不执行普通 JavaScript。补充 assigned JSON script 图片 content 单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只扩大 provider 响应解析兼容面。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider 文本/HTML script 响应解析，`collect_images_from_unknown()` 支持 JSONP 或函数包裹 JSON 片段，例如 `callback({"resultUrl":"/outputs/a.png"});`，会提取括号内 JSON 并递归识别图片字段；仍不执行普通 JavaScript。补充 JSONP 图片 content 单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只扩大 provider 响应解析兼容面。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider HTML 文本响应解析，`extract_image_urls_from_text()` 支持 `poster` 和 `background` 属性里的图片候选，可从视频封面或旧式背景图属性中提取相对结果图；普通非图片候选仍会被过滤。相对路径继续按 provider base URL 解析下载，下载后走 Content-Type、大小和图片签名校验。补充 HTML poster/background 图片属性单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只扩大 provider 响应解析兼容面。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider HTML 文本响应解析，`extract_image_urls_from_text()` 支持 `<source src>`、`<object data>` 和 `<embed src>` 里的图片候选，可从资源页、对象嵌入或兼容页面里提取相对结果图；普通脚本 `src` 等非目标标签仍不会进入候选。相对路径继续按 provider base URL 解析下载，下载后走 Content-Type、大小和图片签名校验。补充嵌入式 HTML 图片属性单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只扩大 provider 响应解析兼容面。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider 文本响应 URL 提取，`extract_image_urls_from_text()` 支持普通文本里的协议相对图片 URL（如 `//cdn.example.test/outputs/a.png`），会沿用 provider base URL 的 `http/https` scheme 下载；JSON 字段内协议相对 URL 的既有解析保持不变，尾随标点清理继续生效。补充文本协议相对 URL 下载单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只提升 provider 文本响应解析容错。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider 文本响应 URL 提取，`extract_image_urls_from_text()` 支持 `https:\u002F\u002Fexample.test\u002Foutputs\u002Fa.png` 这类 Unicode 转义斜杠 URL，避免代理把图片链接作为 JSON 字符串片段输出时无法识别；既有 `\/` 转义斜杠、协议相对 URL 和尾随标点清理保持不变。补充 Unicode 转义斜杠 URL 单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只提升 provider 文本响应解析容错。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider 文本响应 URL 提取，`extract_image_urls_from_text()` 支持 `https\u003A\u002F\u002Fexample.test\u002Foutputs\u002Fa.png` 这类冒号和斜杠同时 Unicode 转义的图片 URL，适配代理把完整链接按 JSON 转义片段输出的情况；既有普通 URL、`\/`、`\u002F` 和协议相对 URL 解析保持不变。补充 Unicode 转义冒号 URL 单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只提升 provider 文本响应解析容错。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider 文本响应 URL 提取，`extract_image_urls_from_text()` 支持图片 URL 查询串里的 Unicode 转义字符，能将 `\u003F`、`\u003D`、`\u0026` 还原为 `?`、`=`、`&`，适配签名图片链接或带尺寸参数链接被代理按 JSON 片段输出的情况；既有普通 URL、转义斜杠和协议相对 URL 解析保持不变。补充 Unicode 转义查询串 URL 单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只提升 provider 文本响应解析容错。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider 文本响应 URL 清理，`clean_image_url()` 会剥离裸图片 URL 末尾常见闭合括号 `]`、`}` 以及 ASCII 冒号，避免 `result [https://.../a.png]` 或 `{https://.../a.webp}` 这类包装文本把括号带进下载地址；既有 Markdown、HTML、转义 URL 和尾随标点清理保持不变。补充尾随括号 URL 单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只提升 provider 文本响应解析容错。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider 文本响应 URL 提取，`extract_image_urls_from_text()` 支持普通文本中内联的相对图片路径或文件名，例如 `done outputs/a.webp`、`preview generated.png`，适配代理只返回一句自然语言结果但未使用 Markdown/HTML 包装的情况；非图片扩展名仍不会进入下载候选。相对路径继续按 provider base URL 解析下载并走图片签名校验。补充内联相对图片路径单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只提升 provider 文本响应解析容错。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider 文本响应 URL 提取，`extract_image_urls_from_text()` 支持签名图片 URL 里 `\u002B` 和 `\u0023` 的 Unicode 转义还原，可将查询串里的 `+` 与 fragment `#` 正常带入下载候选；既有 `\u003F`、`\u003D`、`\u0026`、转义斜杠和协议相对 URL 解析保持不变。补充 Unicode 转义加号和 fragment URL 单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只提升 provider 文本响应解析容错。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider 通用资源字段兼容，`collect_images_from_unknown()` 新增识别 `previewUrl/preview_url`、`thumbnailUrl/thumbnail_url`、`thumbUrl/thumb_url`、`secureUrl/secure_url`、`sourceUrl/source_url`、`originalUrl/original_url` 等常见图片链接字段，适配文件代理、图库代理或 CDN 聚合服务返回预览图、缩略图和安全链接的格式；相对路径仍按 provider base URL 下载并继续走图片签名校验。补充预览/缩略图字段别名单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只扩大 provider 响应解析兼容面。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider 通用 base64 响应字段兼容，`collect_images_from_unknown()` 新增识别 `b64`、`base64Data/base64_data`、`imageB64/image_b64` 等常见代理字段；仍只在内容通过图片签名校验后返回，重复图片继续去重。扩展 base64 字段别名单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只扩大 provider 响应解析兼容面。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider 通用资源字段兼容，`collect_images_from_unknown()` 新增识别 `fileUrl/file_url`、`assetUrl/asset_url`、`artifactUrl/artifact_url`、`downloadURL` 等常见文件/资产链接字段，适配文件代理或存储服务返回资源对象的格式；相对路径仍按 provider base URL 下载并继续走图片签名校验。补充文件/资产 URL 字段别名单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只扩大 provider 响应解析兼容面。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。

2026-07-05：继续增强 provider 文本响应 URL 提取，`extract_image_urls_from_text()` 支持 JS hex escape 形式的图片 URL，例如 `https\x3A\x2F\x2Fexample.test\x2Foutputs\x2Fa.png\x3Fsig\x3Da\x2Bb\x23preview`，会还原 `\x2F/\x3A/\x3F/\x3D/\x26/\x2B/\x23` 后继续提取下载；既有 Unicode 转义、JSON 转义斜杠和协议相对 URL 解析保持不变。补充 hex 转义 URL 单测。本轮不新增配置项，不影响 `_conf_schema.json`、Web API、图片缓存清理、生成记录结构或用户额度；只提升 provider 文本响应解析容错。验证命令：`python -m unittest tests/test_core.py`、`python -m py_compile __init__.py constants.py generator.py main.py models.py persona.py preset.py providers.py utils.py web.py`。
