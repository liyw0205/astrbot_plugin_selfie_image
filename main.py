...
LEGWEAR_PROMPTS = {
    "光腿神器": (
        "本次腿部穿搭：光腿神器。"
        "自然肤色、轻薄透气的光腿效果，干净匀净，不要厚实打底裤感。"
    ),
    "白丝": (
        "本次腿部穿搭：白丝。"
        "纯白轻薄中筒丝袜，袜口在大腿中段，完整包住小腿与脚部，大腿根以上露肤；"
        "袜口自然贴合，有轻微真实贴肤感，不要涂色感或连裤全包。"
    ),
    "黑丝": (
        "本次腿部穿搭：黑丝。"
    ...
    def _build_leg_focus_action(
        ...
        *,
        ...
        if feet_cropped:
            ...
            anatomy_rules += "不穿鞋；脚部朝向自然，腿部比例正常。"
        ...
        if feet_cropped:
            if legwear == "光腿神器":
                legwear_rule = "本次腿部穿搭：光腿神器。大腿自然通透干净，不要厚重打底感。"
            else:
                color = "纯白" if legwear == "白丝" else "纯黑"
                legwear_rule = (
                    f"本次腿部穿搭：{legwear}。{color}轻薄左右分离的大腿袜，袜口在大腿上段自然贴合；"
                    "不是连裤袜或打底裤。袜身向画外延伸，不展示脚部。"
                )
        base = (
            "看看腿。"
