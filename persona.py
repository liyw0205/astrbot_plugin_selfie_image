    ...
    def build_selfie_prompt(
        ...
        if intent.is_group_photo:
            ...
            mode_lines.extend(
                [
                    ...
                    "腿部肤质要干净细腻、自然通透，像「光腿神器」后的舒服观感：匀净柔光、无毛糙、无瑕疵斑点、不假白不油腻；可光腿，也可极薄肉色丝袜/过膝袜/堆堆袜/长筒袜。",
                    "袜装、鞋子和衣服优先按用户本次要求；未指定时按动作描述：部分姿势光腿更好看（侧躺、跪坐、站立俯视、窗台等），部分姿势可过膝袜/堆堆袜；不要张张都穿袜。避免只盖到脚踝的短袜。鞋可选小皮鞋或居家拖鞋。",
                    # 姿势只由代码侧 _build_leg_focus_action 单选写入 action，此处禁止再列姿势菜单以免模型混姿势。
        ...
        if intent.is_group_photo:
            ...
            output_lines = [
                ...
                "3. 镜头与姿势严格跟动作描述里的「本次主姿势」；膝盖、小腿、脚踝和鞋袜清晰。",
                "4. 未指定袜装时按动作描述：可光腿或过膝袜/堆堆袜，不要默认每张都穿袜；避免又短又紧的短袜；用户指定短袜/光腿/丝袜时以用户为准。",
                "5. 人体结构自然完整，仅两条腿两只脚；皮肤与鞋袜干净细腻。",
