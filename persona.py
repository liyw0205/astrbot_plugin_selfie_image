...
def anatomy_constraint_lines(*, style: str = "general") -> list[str]:
    ...
    if style == "legs":
        return [
            "单人限定：画面里只有主角一人。",
            "肢体完整：两条腿、两只脚，左右各一；髋-膝-踝连续，关节朝向正常。",
            "姿势像日常随手拍：重心稳定、可维持，不要拧翻、反折或不可能体位。",
            "脚部朝向自然正位；可见手与肩肘腕连续，自然放在腿侧或膝附近。",
        ]
    return [
            ...
            if feet_cropped:
                mode_lines.extend(
                    [
                        ...
                        "双腿从髋到膝连续自然，左右各一；屈膝坐姿重心稳定，日常可维持。",
                        "腿部穿搭只用动作描述已选的光腿神器、白丝或黑丝；白丝/黑丝为不透的左右分离大腿袜，主要看腿形。",
                        "严格执行动作描述构图；不补画脚部或鞋子。",
                ...
                mode_lines.extend(
                    [
                        ...
                        "光腿神器自然通透；白丝/黑丝为不透的中筒丝袜，遮住肤色、主要看腿形，袜口在大腿中段并完整包住脚部。不要超薄透视，不要连裤全包，不穿鞋。",
        ...
        if intent.is_group_photo:
            if appearance_type == "anime":
                output_lines = [
                    ...
                    "2. 每张额外参考图对应独立同框人物，统一为柔光京阿尼风格；脸更细、气质更仙风；真人参考只取身份线索再画成同风格动漫人物；玩偶/毛绒/吉祥物/道具必须拟人成完整动漫人物，禁止原样玩具外形或贴在主角身上。",
                    "3. 拟人无明确性别时默认成年女性；有明确性别线索则按对应性别。主角性别仍跟随参考图一。",
                    "4. 所有人物在同一完整场景中，站位或坐位自然，比例与透视一致；人物彼此边界清晰，禁止身体/衣物粘连融合。",
                    "5. 整张图是一张统一柔光京阿尼画风的动漫合影，不要一半真人一半二次元。",
            ...
            if feet_cropped:
                output_lines = [
                    ...
                    "3. 双腿从髋到膝连续，屈膝坐姿重心稳定、日常可维持。",
                    "4. 穿搭按动作描述选用光腿神器、白丝或黑丝；白丝/黑丝为不透的左右分离大腿袜，主要看腿形。",
                    "5. 保持日常完整照片效果。",
                ]
            else:
                output_lines = [
                    ...
                    "3. 姿势跟动作描述，日常可维持；髋-膝-踝连续，不穿鞋，脚部自然完整。",
                    "4. 穿搭按动作描述选用光腿神器、白丝或黑丝；中筒丝袜不透、完整包住脚部，主要看腿形；不要超薄透视，不要连裤全包。",
                    "5. 腿部结构自然完整，关节朝向正常。",
                ]
        else:
            output_lines = [
