...
class PersonaManager:
    ...
    def build_selfie_prompt(
        ...
        identity_lines = (
            [
                "固定形象参考：参考图一是主角身份，拥有最高优先级。",
                "主角的脸、五官、发型、发色、气质、体态和整体形象必须来自参考图一；角色名称和人设只辅助语气与氛围。",
                "后续额外参考图不得替换主角身份、脸部、五官、发型、发色、体态或整体长相。",
                "若参考图一里主角正对镜头却眼神飘向别处或心不在焉，生成时改为自然看向镜头、眼神有焦点；身份仍锁参考图一，只修正视线与表情专注度。",
            ]
            ...
            if intent.is_group_photo:
                reference_lines.extend(
                    [
                        ...
                        "人物参考：保留可见人数、脸型五官倾向、发型发色、穿搭轮廓、体态站位与相对关系。",
                        "风景、建筑、房间、道具、表情包、卡通、吉祥物、纯色块等非人物图：也要按该图的主色、线条、材质、符号和气质，拟人成可并肩站立的完整人物角色（成年、得体、自然），再与主角同框；不要把原图原样铺成背景。",
                        "多张额外参考图可各自对应一个独立同框角色；场景统一由最终合影自行组织，光线与画风一致。",
                    ]
                )
            else:
                reference_lines.extend(
                    [
                        ...
                        "若额外参考图有遮挡面部的物件或动作，默认不要照搬；除非用户明确要求遮脸，否则主角面部清晰自然。",
                    ]
                    if has_reference_image
                    else [
        ...
        if intent.is_group_photo:
            mode_lines.extend(
                [
                    "【合影 / 同框模式】",
                    "先确定你自己的形象，再把每张额外参考图落实为独立的同框人物。",
                    "风景、建筑、房间、道具、表情包、卡通、吉祥物等非人物参考：按风格拟人成可互动的完整人物，保留主色与标志特征；禁止仅当背景贴图。",
                    "同框人物自然站位或坐位，距离、遮挡、视线和互动关系合理。",
                    "所有人物在同一场景，光线、色调、画风和相机透视统一。",
                    "整体像同一时间、同一地点、同一相机拍下的一张日常合影。",
                    "合影默认多数人看向镜头（或看向画面中的相机方向），表情自然、有互动；除非用户明确要求看向彼此或看向别处，不要全员心不在焉、眼神飘走。",
            ...
            mode_lines.extend(
                [
                    "【特写自拍 / 晒腿模式】",
                    "成年角色自然坐姿随手拍，构图重点放在腿部线条、袜装、鞋面和居家衣料细节上，画面得体、日常、柔和。",
                    "即使脸部不入镜，也必须保持参考图一的主角身份：发色、发型露出的部分发丝、肤色、体态、手部、穿搭气质和整体氛围都像同一个角色。",
                    "袜装、鞋子和衣服优先按用户本次要求；未指定时可在白色/黑色/灰色长筒袜、过膝袜、堆堆袜、小皮鞋、居家拖鞋等日常搭配中自然选择。",
                    "优先第一人称俯视视角（POV，低头看自己的腿）、窗边坐姿自拍、床沿坐姿、地毯坐姿或单人椅侧斜坐姿，不要每次都固定同一个姿势。",
                    "腿部姿态可为双腿向前放松、斜侧摆放、轻微交叠、并拢放松、一只脚微微前伸、脚尖轻微内扣或自然朝外；膝盖、脚尖和坐姿重心必须协调。",
                    "画面重点呈现裙摆/裤脚、膝盖、小腿、脚踝、鞋袜搭配、衣料垂落、袜口、鞋面材质、地毯/床单/木地板纹理和手部轻扯裙边/整理袜口等生活化细节。",
                    "脸部保持在画面外，构图集中在下半身、腿部和周围居家环境。",
                    "手部可自然整理裙摆、衣角、袜口或鞋带，也可以轻扶膝盖；动作含蓄放松。",
                    "环境和光线跟当前时间段变化：晨光、午后漫反射、傍晚暖灯、夜里床边小灯、居家地毯、沙发边、窗边、床单、木地板或浅色系房间。",
                    "脚背、脚踝和腿部皮肤要干净细腻；结构自然完整，不要脏污脚面、畸形脚趾或不自然脚踝。",
                    "画面干净、自然、写实，有温柔的日常随手拍氛围。",
                ]
            )
            ...
            mode_lines.extend(
                [
                    ...
                    "可以看向镜头、轻松回头，或偶尔看向别处做自己的事；若画面是正面半身/近景，优先看向镜头，眼神自然有焦点，不要整段心不在焉。",
        ...
        if intent.is_group_photo:
            output_lines = [
                ...
                "2. 每张额外参考图对应独立同框人物：人物图保留外观与站位关系；风景/道具/卡通/表情包等按风格拟人成完整人物，不要当背景贴图。",
                "3. 所有人物在同一完整场景中，站位或坐位自然，比例与透视一致。",
                "4. 整张图像像真实拍下的一张日常合影，统一光线、色调、景深和相机视角。",
                "5. 人体结构自然完整；多人时身份清晰，脸、发型、服装、体态各自独立。",
                "6. 同框角色均为得体、日常的成年形象，互动自然友好。",
                "7. 合影时默认看向镜头（有眼神交流）；参考图若眼神飘忽，生成时修正为自然对镜，除非用户另有要求。",
            ]
        elif intent.is_legs_only:
            output_lines = [
                "【生成要求】",
                "1. 主角身份稳定，来自参考图一或角色设定。",
                "2. 构图集中在下半身、腿部线条、袜装、鞋面、衣料垂落和居家材质，脸部保持在画面外，但发丝、手部、肤色、体态和气质仍要来自参考图一。",
                "3. 镜头语言：第一人称俯视、窗边/床沿/地毯/单人椅坐姿自拍中自然变化，膝盖、小腿、脚踝和鞋袜清晰。",
                "4. 人体结构自然完整，腿部比例、坐姿重心、手部互动、衣料和光影关系可信；皮肤与鞋袜干净细腻。",
            ...
            output_lines = [
                ...
                "3. 除非用户明确要求遮脸，否则主角面部无遮挡、清晰可见，脸和五官保持参考图一或角色设定。",
                ...
                "7. 参考图若正对镜头却眼神飘走，生成时改为自然对镜，只改视线与专注度，不改身份。",
