...
def anatomy_constraint_lines(*, style: str = "general") -> list[str]:
    ...
    if style == "legs":
        return [
            ...
            "双脚/脚踝朝向自然正位，不要单脚外翻、脚跟外拧、脚掌翻折或反关节。",
            "腿部比例自然：大腿不要异常粗圆，膝有正常骨感，小腿与大腿协调；俯视要有近大远小。",
            "脚趾五个分开清晰完整，不要黏连融合、脚掌融化或脚趾畸形。",
...
class PersonaManager:
    ...
    def build_selfie_prompt(
        ...
        feet_cropped = "【pose:reclined_knees_crop】" in act
        ...
        if intent.is_group_photo:
            ...
            if feet_cropped:
                mode_lines.extend(
                    [
                        "【特写自拍 / 晒腿模式 / 脚部画外】",
                        "单人近距离俯拍：只拍少量腰腹/裙摆、大腿与膝部，脚踝和双脚完整裁出画外。",
                        "即使脸部不入镜，也必须保持参考图一的主角身份：发色、肤色、体态与整体气质像同一个角色。",
                        "双腿从髋部到膝盖连续自然，左右各一；双膝弯曲抬起，方向一致，边界清楚，不交叉融合或反折。",
                        "腿部穿搭只使用动作描述中选定的光腿神器、白丝或黑丝；白丝/黑丝必须是左右分离的大腿袜，袜口在大腿上段，禁止连裤袜或一体全包袜。",
                        "严格执行动作描述中的后仰屈膝构图；不要补画脚尖、脚底、鞋子或任何脚部细节。",
                        "近距离第一人称俯视，允许一条腿略靠近镜头形成自然近大远小；不要强行对称。",
                        "手部尽量不入镜；若出现，必须与肩肘腕连续，只可轻放腰腹或裙摆附近。",
                        "画面得体、日常、柔和，光线、色调、景深和相机透视统一。",
                    ]
                )
            else:
                mode_lines.extend(
                    [
                        ...
                        "光腿神器要自然通透像干净裸腿；白丝/黑丝必须是轻薄半透明中筒丝袜，袜口到大腿中段并勒进大腿肉（有压痕与轻微鼓肉），包住脚踝与整只脚到脚趾，大腿根以上露肤；要有贴合拉伸与自然褶皱，不要涂色感；禁止连裤/全包/露趾半截。赤足无鞋。",
                        "姿势规则：严格只执行动作描述里的「本次主姿势」与「本次腿部特写构图」；姿势日常合理、重心稳定，不要僵硬拧腿、单脚外翻、脚踝拧翻或不可能体位；不要同时出现坐姿+跪姿+侧躺等多种姿态。",
                        "优先第一人称俯视或下半身近景，构图以腿部为主，近大远小；若边缘出现发丝/肩颈，须与身体自然相连，不要悬浮错位的头。",
                        ...
                        *anatomy_constraint_lines(style="legs"),
                        ...
                        "赤足无鞋：脚背脚踝干净；脚趾五个分开自然完整，不要黏连融合、脚掌融化或畸形脚趾。",
                        "腿部体积自然：大腿不要粗圆柱，膝有正常骨感，小腿与大腿比例协调。",
        ...
        if intent.is_group_photo:
            if appearance_type == "anime":
                output_lines = [
                    ...
                    "2. 每张额外参考图对应独立同框人物，统一为动漫/二次元画风；真人参考只取身份线索再画成动漫人物；玩偶/毛绒/吉祥物/道具必须拟人成完整动漫人物，禁止原样玩具外形或贴在主角身上。",
                    "3. 拟人无明确性别时默认成年女性；有明确性别线索则按对应性别。主角性别仍跟随参考图一。",
                    "4. 所有人物在同一完整场景中，站位或坐位自然，比例与透视一致；人物彼此边界清晰，禁止身体/衣物粘连融合。",
                ...
                output_lines = [
                    ...
                    "2. 每张额外参考图对应独立同框人物：真人照片保留大致外观；二次元/插画/表情包/卡通/玩偶/毛绒只取发色发型配色等线索，改画成写实真人；风景/道具按风格拟人成完整写实人物；禁止原样玩具外形、平面贴图或贴在主角身上。",
                    "3. 拟人无明确性别时默认成年女性；有明确性别线索则按对应性别。主角性别仍跟随参考图一。",
                    "4. 所有人物在同一完整场景中，站位或坐位自然，比例与透视一致；人物彼此边界清晰，禁止身体/衣物粘连融合。",
                ...
                output_lines = [
                    "【生成要求】",
                    "1. 主角是你自己：性别、脸型五官、发型发色、体态来自参考图一或角色设定；参考图是女性则必须是女性，禁止改成男性；表情眼神按本次合影氛围自然重画。",
                    "2. 每张额外参考图对应独立同框完整人物，画风与主角统一；人物参考取身份线索，玩偶/毛绒/吉祥物/道具/非人物参考必须拟人成完整人物，禁止原样玩具外形、贴图或贴在主角身上。",
                    "3. 拟人无明确性别时默认成年女性；有明确性别线索则按对应性别。主角性别仍跟随参考图一。",
                    "4. 所有人物在同一完整场景中，站位或坐位自然，比例与透视一致；人物彼此边界清晰，禁止身体/衣物粘连融合。",
            ...
            if feet_cropped:
                output_lines = [
                    "【生成要求】",
                    "1. 主角身份稳定，来自参考图一或角色设定；画面只有主角一人。",
                    "2. 竖幅近距离俯拍，只露少量腰腹/裙摆、大腿与膝部；脚踝和双脚完整裁出画外，不补画脚尖、脚底或鞋。",
                    "3. 双腿从髋部到膝盖连续自然，左右各一，双膝屈起方向合理；不要交叉融合、反折或多出第三条腿。",
                    "4. 腿部穿搭严格采用动作描述中选定的光腿神器、白丝或黑丝；白丝/黑丝是左右分离的大腿袜，袜口在大腿上段，禁止连裤袜、一体全包袜或打底裤。",
                    "5. 近距离俯视保留自然近大远小，一条腿可略靠近镜头；不要强行对称或夸张拉长。",
                ...
                output_lines = [
                    ...
                    "3. 镜头与姿势严格跟动作描述里的「本次主姿势」；姿势日常合理，双脚不外翻；膝盖、小腿、脚踝和脚背清晰；赤足无鞋。",
                    "4. 腿部穿搭严格采用动作描述中选定的光腿神器、白丝或黑丝；白丝/黑丝为到大腿的中筒半透丝袜，袜口勒肉有压痕，且包住整脚到脚趾；不要连裤全包、不要露趾半截、不要涂色感；不要任何鞋子。",
                    "5. 人体结构自然完整，仅两条腿两只脚，左右各一只；大腿体积自然、膝有骨感、脚趾分开不黏连；手若入镜须与胳膊连续连接。",
                    "6. 保持单张完整照片效果，统一光线、色调、景深和相机透视。",
                ]
        else:
            output_lines = [
                "【生成要求】",
                "1. 主角身份稳定：脸型五官、发型发色、体态来自参考图一或角色设定。",
                ...
                + (
                    ...
                    else "自然自拍：看向镜头、眼神有焦点，手臂与构图协调；不要心不在焉看向别处（用户明确要求除外）。"
                ),
