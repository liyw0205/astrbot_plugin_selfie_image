...
def anatomy_constraint_lines(*, style: str = "general") -> list[str]:
    ...
    if style == "legs":
        return [
            ...
            "脚部朝向自然正位；若脚部入镜，脚趾形态正常清晰、不黏连；可见手与肩肘腕连续，自然放在腿侧或膝附近。",
        ]
    return [
        ...
        if intent.is_cos_look:
            mode_lines.extend(
                [
                    ...
                    "全身或半身完整入镜，重点还原套装层次、颜色、配饰与开叉/薄纱结构；不要裁成只拍腿或只拍脸。",
                    "构图以展示 COS 服装为主，日常得体；表情按新造型自然重画。",
                ]
            )
            ...
            if feet_cropped:
                mode_lines.extend(
                    [
                        "【特写自拍 / 晒腿模式 / 脚部画外 / 不露脸】",
                        "单人近距离俯拍：只拍少量腰腹/裙摆、大腿与膝部；双脚裁出画外，脸部也裁出画外。",
                        "保持参考图一主角身份：体态、肤色气质一致（本张不需要脸）。",
                        "双腿从髋到膝连续自然，左右各一；屈膝坐姿重心稳定，日常可维持；禁止膝盖顶脸。",
                        "腿部穿搭只用动作描述已选的光腿神器、白丝或黑丝；白丝/黑丝为不透的左右分离大腿袜，袜口有卷边，主要看腿形。",
                        "严格执行动作描述构图；不补画脚部、鞋子或脸部。",
        ...
        if intent.is_cos_look:
            ...
            if feet_cropped:
                output_lines = [
                    "【生成要求】",
                    "1. 主角身份稳定，画面只有主角一人。",
                    "2. 近距离俯拍只露腰腹/裙摆到膝部；双脚裁出画外，脸部也裁出画外。",
                    "3. 双腿从髋到膝连续，屈膝坐姿重心稳定、日常可维持；禁止膝盖顶脸。",
                    "4. 穿搭按动作描述选用光腿神器、白丝或黑丝；白丝/黑丝为不透的左右分离大腿袜，袜口有卷边，主要看腿形。",
                    "5. 保持日常完整照片效果，焦点在腿形。",
                ]
            else:
                output_lines = [
