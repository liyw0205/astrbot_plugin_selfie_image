    ...
    def build_selfie_prompt(
        ...
        if intent.is_cos_look:
            ...
            legwear_line = (
                ...
                "禁止中筒袜、短袜等停在小腿中段的普通袜型，膝部以下不入镜。"
                if selected_text
                else "腿部穿搭只允许光腿神器、白丝或黑丝三选一；不参考参考图中的袜子或其他腿部穿搭；禁止中筒袜、短袜等停在小腿中段的普通袜型，膝部以下不入镜。"
            )
            mode_lines.extend(
                [
                    f"【服装局部展示 / {'第一人称手机自拍' if camera_kind == 'selfie' else '第三人称摄影'}】",
                    camera_line,
                    "严格近距离取景：画面只有主角一人，只保留腰线附近到膝部附近的服装区域，完整人物在画外，不扩展为半身或全身。",
