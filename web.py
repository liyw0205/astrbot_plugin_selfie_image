...
<body>
  <div id="loginPage" class="login-page">
    <div class="login-box">
      ...
      <p class="muted" id="loginHint">独立 Flask 入口需要 Web Token。AstrBot 内嵌页会自动进入，无需输入 Token。</p>
  ...
  <main class="app-shell">
    ...
    <section id="test">
      ...
      <div class="grid">
        <label class="checkline"><input id="promptEnhance" type="checkbox"> 提示词增强</label>
  ...
  <div id="channelModal" class="modal-mask">
    <div class="modal">
      ...
      <div class="grid">
        ...
        <div><label>API Key（可多行，每行一个；401/429 自动轮询下一把）</label><textarea id="modalApiKey" rows="3" placeholder="sk-xxx&#10;sk-yyy"></textarea></div>
  ...
  <script>
      ...
      try {
        ...
        if (task.status === 'queued' || task.status === 'running') {
          ...
          const hint = seconds >= 20
            ? ' gpt-image 中转常需 15–60 秒，请继续等待；超过全局超时才会失败。'
            : '';
          $('testStatus').textContent = `后台任务 ${task.task_id || TEST_TASK_ID} ${label}，已用 ${seconds}s。${hint}关闭页面不会停止任务。`;
    ...
    (async function init() {
      ...
      if (embedded) {
        if ($('loginPage')) $('loginPage').style.display = 'none';
        if ($('loginStatus')) $('loginStatus').textContent = '正在连接 AstrBot 内嵌管理通道...';
        if ($('loginHint')) $('loginHint').textContent = 'AstrBot 内嵌页复用 Dashboard 登录态，无需 Web Token。';
        try {
