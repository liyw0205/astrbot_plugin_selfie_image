...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .studio-gallery-item img { width:100%; height:72px; object-fit:cover; border-radius:6px; display:block; }
...
<body>
  ...
  <main class="app-shell">
    <nav class="page-nav" aria-label="功能分区">
      ...
      <button data-tab="proxies" type="button">代理</button>
    ...
    <section id="channels">
      <div class="between">
        ...
        <div class="actions" style="margin-top:0">
          ...
          <button class="secondary" type="button" onclick="addAuditChannel()">加辅助渠道</button>
    ...
    <section id="proxies">
      <div class="between">
        <h2>代理</h2>
        <div class="actions" style="margin-top:0">
          <button class="secondary" type="button" onclick="renderProxyList()">刷新</button>
          <button class="ok" type="button" onclick="openProxyModal(-1)">添加代理</button>
        </div>
      </div>
    ...
    <section id="studio">
      <div class="between">
        <h2>画布</h2>
        <span class="status">多参考编排 · 多模板 · 仅图片</span>
      </div>
      <div class="grid">
        <div>
          <label>画布会话</label>
        ...
        <div>
          <label>标题</label>
          <input id="studioTitle" placeholder="画布标题">
        </div>
        <div class="actions" style="align-items:end">
          <button class="ok" id="studioCreateBtn" type="button">按模板新建</button>
      ...
      <div class="between" style="margin-top:8px">
        <h3>参考槽位</h3>
        <div class="actions">
          <button class="secondary" id="studioPickRecordBtn" type="button">从记录选图</button>
          <button class="secondary" id="studioAddSlotBtn" type="button">加槽位</button>
        </div>
      </div>
      <div class="studio-gallery" id="studioGallery"></div>
    ...
    <section id="image">
      ...
      <div class="grid4">
        ...
        <div><label>同时画几张上限</label><input id="maxConcurrent" type="number" min="1" max="10" title="同一时间最多几张一起请求上游，硬上限 10；新任务自动排队"></div>
...
{prompt}`;
    ...
    function resolveModelProviderType(model, defaultProviderType, manualProviderType = '') {
      ...
      // openai/openai_chat/gemini_openai channels keep default for gemini-like names (protocol lock).
    ...
    async function downloadCachePath(path, filename) {
      ...
      try {
        showToast('正在准备下载…', 'ok');
        if (isDashboardPage()) {
          // Binary download via bridge.
          const bridge = await waitForDashboardBridge();
          if (typeof bridge.ready === 'function') await bridge.ready();
          if (typeof bridge.download !== 'function') throw new Error('当前页面不支持文件下载');
    ...
    function renderStudioSlots() {
      ...
      slots.forEach((slot, index) => {
        ...
        card.innerHTML = `
          <div class="slot-label">${escapeHtml(slot.label || slot.role || '槽位')} · ${escapeHtml(slot.role || '')}</div>
          ...
          <div class="slot-actions">
            ...
            <button type="button" class="secondary mini" data-act="record">记录</button>
    ...
    function renderStudioResults() {
      ...
      for (const item of session.results || []) {
        ...
        card.innerHTML = `
          ...
          <div class="status" style="font-size:11px">${escapeHtml(item.created_at || '')}${item.used_model ? ' · ' + escapeHtml(item.used_model) : ''}</div>`;
    ...
    async function openStudioGallery(slotId) {
      ...
      if (!res.success) {
        panel.innerHTML = `<div class="status">${escapeHtml(res.message || '加载失败')}</div>`;
        return;
      }
      const items = (res.data && res.data.items) || [];
      if (!items.length) {
        panel.innerHTML = '<div class="status">暂无成功记录图</div>';
        return;
      }
      const targetHint = STUDIO.galleryTargetSlotId
        ? '点击图片放入当前槽'
        : '先点槽位「记录」，或点图后选默认空槽/形象槽';
      panel.innerHTML = `<div class="between"><div class="status">${targetHint}</div>
        <button type="button" class="secondary mini" id="studioGalleryClose">收起</button></div>
        <div class="studio-gallery-grid" id="studioGalleryGrid"></div>`;
      $('studioGalleryClose').onclick = () => { panel.classList.remove('open'); panel.innerHTML=''; STUDIO.galleryTargetSlotId=''; };
      const grid = $('studioGalleryGrid');
      for (const item of items) {
        const cell = document.createElement('div');
        cell.className = 'studio-gallery-item';
        cell.innerHTML = `<img data-cache-path="${escapeHtml(item.path || '')}" alt=""><div class="cap">${escapeHtml(item.prompt || item.model || item.path || '')}</div>`;
        cell.onclick = () => studioPickGalleryPath(item.path);
        grid.appendChild(cell);
      }
      loadProtectedImages(grid);
    }
    async function studioPickGalleryPath(imagePath) {
    ...
    function renderStudio() {
      ...
      if (!s) {
        $('studioTitle').value = '';
        $('studioPrompt').value = '';
        $('studioStatus').textContent = '选模板后点「按模板新建」，或打开已有会话。';
        renderStudioSlots();
        renderStudioResults();
        return;
      }
    ...
    document.querySelectorAll('nav.page-nav button').forEach(btn => {
      btn.onclick = () => {
        ...
        if (btn.dataset.tab === 'proxies') { loadConfig().then(() => renderProxyList()).catch(() => renderProxyList()); }
      };
    });
