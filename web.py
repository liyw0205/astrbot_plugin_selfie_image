...
<body>
  ...
  <script>
    ...
    // OmniDraw-style video protocols: async poll / long sync / chat returns URL
    const VIDEO_PROVIDERS = ['video_async','video_sync','video_chat'];
    const VIDEO_PROVIDER_LABELS = {
      video_async: '异步任务（/videos/generations + 轮询）',
      video_sync: '同步等待（长连接出片）',
      video_chat: '对话接口（chat/completions 回链）'
    };
    ...
    function normalizeVideoProviderType(value) {
      ...
      if (!raw) return '';
      if (rawText.includes('异步') || rawText.includes('轮询')) return 'video_async';
      if (rawText.includes('同步')) return 'video_sync';
      ...
      const aliases = {
        ...
        // bare openai stays image protocol; video channel builder maps it to video_async
      };
      const normalized = aliases[raw] || raw;
      return VIDEO_PROVIDERS.includes(normalized) ? normalized : '';
    }
    ...
    function inferVideoProviderTypeFromModel(model) {
      const compact = String(model || '').trim().toLowerCase().replace(/[\s_]+/g, '-');
      if (!compact) return '';
      if (['chat','gpt-4o','gpt4o','claude','deepseek'].some(k => compact.includes(k))) return 'video_chat';
      if (['luma','runway','kling','可灵','minimax','hailuo','vidu','sora','cogvideo','wanx','wan2'].some(k => compact.includes(k))) return 'video_async';
      if (compact.includes('sync')) return 'video_sync';
      return '';
    }
    ...
    function normalizeVideoChannels() {
      CONFIG.video_channels = (CONFIG.video_channels || []).map(ch => {
        ch = normalizeChannel(ch);
        ch.provider_type = normalizeVideoProviderType(ch.provider_type) || 'video_async';
    ...
    function newVideoChannel() {
      return normalizeChannel({name:'video-channel', provider_type:'video_async', base_url:'https://api.openai.com/v1', api_key:'', model:'', enabled_models:[], timeout:300, enabled:false, models_cache:[]});
    }
    ...
    function collectModalChannel() {
      ...
      const ch = normalizeChannel(Object.assign({}, source, {
        ...
        provider_type: EDITING_CHANNEL_KIND === 'video'
          ? (normalizeVideoProviderType($('modalProvider').value || source.provider_type) || 'video_async')
    ...
    function openChannelModal(index, kind = 'image', options = {}) {
      ...
      if (isNew) {
        ...
        if (EDITING_CHANNEL_KIND === 'video') ch.provider_type = normalizeVideoProviderType(ch.provider_type) || 'video_async';
      ...
      const defaultProvider = EDITING_CHANNEL_KIND === 'video'
        ? (normalizeVideoProviderType(ch.provider_type) || 'video_async')
    ...
    function modalProviderChanged() {
      const ch = currentModalChannel();
      if (EDITING_CHANNEL_KIND === 'video') {
        ch.provider_type = normalizeVideoProviderType($('modalProvider').value) || 'video_async';
      } else {
