...
def build_prompt_with_reference_instruction(
    ...
    *,
    ...
    if language != "en":
        ...
        if not has_cos_contract:
            lines.append(
                (
                    ...
                    "优先保留脸型轮廓和五官比例；姿势、头部动作、视线和表情按本次 COS 套装或用户要求执行，"
                    "面部边缘清晰、肤色自然。"
                    if is_cos
        ...
        return "\n".join(lines)
    lines = ["Use the provided reference image(s) as visual references."]
    if not has_cos_contract:
        lines.append(
            (
                "For COS outfit changes, use the reference for the same person's identity, facial structure, gender, skin tone, and body proportions; "
                "follow the COS outfit or user request for pose, head movement, gaze, and expression."
                if is_cos
